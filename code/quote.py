...
def get_quotes(engine):
    table_name = (datetime.datetime.now() - datetime.timedelta(hours=4)).strftime('%Y-%m-%d %H:%M:%S quote')
    params = {'token':IEX_KEY}
    r = requests.get(f"https://cloud.iexapis.com/stable/tops", params=params)
